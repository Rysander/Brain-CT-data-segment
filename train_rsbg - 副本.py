...
class mIoULoss(nn.Module):
    ...
    def forward(self, inputs, target, loss0 = 100, loss_weight = [0.1,1,1,4,5,5,5,5,3] , epoch = 0):
        ...
        ## Sum over all pixels N x C x H x W => N x erC
        inter = inter.view(N,self.classes,-1).sum(2)

        # get the zero inputs to the average loss
        oneHot2 = oneHot.view(N,self.classes,-1).sum(2)

        # loss2 = inter #+0.0000001)/(oneHot2 + 0.0000001)*oneHot2
        # loss2 = loss2*loss_weight

        inputsoft2 = nn.functional.softmax(inputs,dim=1)
        # inputsoft2 = -oneHot*torch.log(inputsoft2) + oneHot1 * (1/(1-inputsoft2)-1) * (2*epoch + 1)/100
        # loss = inter.view(N,self.classes,-1).sum(2)
        loss = inter*loss_weight
        inputsoft0 = inputsoft2*oneHot
        inputsoft0 = inputsoft0.view(N,self.classes,-1).sum(2)
        inputsoft2 = inputsoft2.view(N,self.classes,-1).sum(2)
        oneHot = oneHot.view(N,self.classes,-1).sum(2)
        inter2 = 1-4*inputsoft0/(3*inputsoft2+oneHot2)
        loss2 = inter2*loss_weight
        ...
        # oneHot3 = 1-oneHot
        # inter2 = -nn.functional.log_softmax(inputsoft2,dim=1) * oneHot3
        ...
        # loss = (inter2 + 0.00000001)/(oneHot3 + 0.00000001)*oneHot3/(oneHot3 + 0.00000001)
        ...
        # loss2 = loss2[:,1:]
        # loss = loss[:,1:]
        # return loss2.mean() + loss.mean()

        return (loss.mean()+(0.5*epoch+20)*loss2.mean())/(1+0.5*epoch+20)
    ...
    for epoch in range(epochs):
        ...
        for iterr, batch in enumerate(train_loader):
            ...
            if (np.max(valuation) > 0) or (epoch % 4 == 3):
                ...
                loss1 = criterion1(outputs, labels , loss0 = loss0,epoch = epoch) #, batch = batch_size)
